...
export default class BareMDE extends Component {
  constructor(props) {
    ...
    if (props.controls) {
      ...
      props.controls.syncScroll = () =>
        this.state.syncScroll && this.syncPreviewScroll();
      props.controls.refreshPreview = this.refreshPreview;
    }
    if (props.imageRewriter) {
      console.info(
        "Image rewriter function is removed, please, do the rewritting in upper level component",
      );
    }
  }
  ...
  async syncPreviewScroll(force) {
    ...
    const doScroll = () => {
      //preview height
      const previewFullH =
        this.previewFrame.current.contentWindow.document.documentElement
          .scrollHeight;
  ...
  toggleFullscreen() {
    ...
    if (v) {
      typeof this.props.onEnterFullscreen === "function" &&
        this.props.onEnterFullscreen();
      this.componentContainer.current.style.zIndex =
        this.props.fullscreenZIndex;
      if (this.props.trueFullscreen && document.fullscreenEnabled) {
        this.componentContainer.current.requestFullscreen();
        this.componentContainer.current.addEventListener(
          "fullscreenchange",
          syncFF,
        );
      }
    } else {
      typeof this.props.onExitFullscreen === "function" &&
        this.props.onExitFullscreen();
      this.componentContainer.current.style.zIndex = "unset";
      if (this.props.trueFullscreen && document.fullscreenEnabled) {
        document.exitFullscreen();
      }
      this.componentContainer.current.removeEventListener(
        "fullscreenchange",
        syncFF,
      );
    }
  ...
  render() {
    ...
    return html`<div
      ...
      style="max-height:${this.state.fullscreen ? "100%" : this.props.maxHeight
      };z-index:${this.state.fullscreen ? this.props.fullscreenZIndex : "initial"
      }"
    >
      <div
        class="toolbar top 
      ...
      >
        <${Menu}
          ...
          zIndex=${this.state.fullscreen
        ? this.props.fullscreenZIndex + 100
        : "initial"
      }
        ...
        <div
          class="bmde_branding"
          dangerouslySetInnerHTML=${{ __html: this.props.branding }}
        />
        <${TButton}
          isOn=${true}
        ...
        <${TButton}
          ...
          title=${this.state.syncScroll
        ? "Turn scroll sync off"
        : "Turn scroll sync on"
      }
        ...
        />
    <${If} condition=${this.props.customButtons !== undefined}>
        <div class="divider" />
      ${this.props.customButtons && this.props.customButtons.map((b) => html`<${TButton} svg=${b.svg} svgOff=${b.svgOff} title=${b.title} onClick=${b.onClick} isOn=${b.isOn} />`)}
    </${If}>
      </div>
      <div class="workArea">
        <div
...
BareMDE.defaultProps = {
  render: (m) =>
    `<html><head></head><body><div style='color:navyblue'>${m}</div></body></html>`,
  renderBody: null,
